...
- 音频上传和两页式父辈页逻辑已形成产品草案：`03_product/product_design_v2.md`；尚待 W 确认。
...
1. 回收 Gemini UI 风格稿，审查父辈可用性，并决定采用方向。
2. 讨论并确认音频上传需求范围。
3. 发布并手机实测粘贴体验、返回首页和成功页布局改动。
4. 为当前可用版本做首次 Git 提交和推送前检查。
5. 明确 `deploy.sh rollback` 在 systemd 管理后的使用边界，避免误用 legacy pid 启停。
6. 旧目录仍可稍后清理；删除前必须再次确认。
