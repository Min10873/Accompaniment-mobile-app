...
| 服务 | 建议 |
|---|---|
| sidecar | `127.0.0.1:7000` |
| FastAPI | `127.0.0.1:7001` |
| 临时 mock FastAPI | `127.0.0.1:7011`，仅短时调试使用 |
| 静态音频 | 由后续反向代理暴露 `/files/audio/` |
| 公网入口 | 后置，不直接假定使用 80/443 |

实际端口需以 W 服务器现状为准，且不得与 x-ui/xray 冲突。
`7000-7999` 作为伴奏应用专属端口段；新的本地调试和下一次生产迁移都按这个区间规划。
...
- sidecar 绑定 `127.0.0.1:7000`
- FastAPI 绑定 `127.0.0.1:7001`
- 不使用 80/443
- 不动现有 VPN/3x-ui/xray
